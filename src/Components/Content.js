...
const Content = () => {
  ...
  const save=()=>{
    if(title==='')alert('Please write note title')
              ...
              dispatch(addNote({title:title,note:note,color:color.substring(0,color.length-1)}))
  ...
  return <>
      ...
      <br />
      <NoteInput placeholder='Write note...' rows={8} value={note} onChange={(e)=>setNote(e.target.value)}/>
      <br />
      <Container className='clearfix'>
        { 
          colorList.map((c,i)=><Color key={i} color={c} onClick={()=>setColor(c)} ><span style={{fontSize:18,display:color===c?'inline':'none'}}>✓</span></Color>)
        }
      ...
      <NoteContainer>
        {
          notes.map((note)=>{
            return <Note key={note.id} color={note.color+")"}>
              <NoteTitle color={note.color+",0.4)"}>{note.title}
                <BsTrash style={{position:'absolute',top:11,right:10,fontSize:20}} onClick={()=>dispatch(removeNote(note.id))}/>
              </NoteTitle>
